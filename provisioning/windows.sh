...
sudo cp /vagrant/provisioning/vagranthosts /tmp
chmod 666 /tmp/vagranthosts

# Run the playbook.
echo "Running Ansible provisioner defined in Vagrantfile."
ansible-playbook -i '/tmp/vagranthosts' /vagrant/${ANSIBLE_PLAYBOOK} --extra-vars "is_windows=true" --connection=local
