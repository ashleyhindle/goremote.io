...
use GoRemote\Model\SourceInterface;
use Knp\Command\Command;
use Symfony\Component\Console\Input\InputArgument;
use Symfony\Component\Console\Input\InputInterface;
use Symfony\Component\Console\Output\OutputInterface;

class CheckSourceCommand extends Command
{
	private $app;
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
        ...
        $tweetString = $tweet ? 'Yes' : 'No';
        $stopOnDuplicateCount = $input->getArgument('stopOnDuplicateCount');

		$output->writeln("Testing Source: {$source}, Tweet: {$tweetString}, DuplicateStop: {$stopOnDuplicateCount}");
		...
		if (!class_exists($className)) {
		    $output->writeln("Class '{$className}' doesn't exist.  Case sensitivity?");
		    return 1;
        }
